...
class Workflow_List_Table extends WP_List_Table {
    ...
    function __construct(){
        ...
        add_action('admin_init', array(&$this, 'get_approval_role'));
    }
    
    function get_approval_role(){
    ...
    function _create_columns(){        
        ...
        $this->_columns = array(
            ...
            new WP_List_Table_Column('reviewer', __('Reviewer', 'approval-workflow'), true)
        );
    }
    ...
    function column_title($item){
        ...
        if ( Approval_Workflow::didIApprove($item['ID']) ){
            $actions['reject'] = '<a href="' . $item['reject_link']. '">' . __('Reject', 'approval-workflow'). '</a>' ;
        } else {
            $actions['approves'] = '<a href="' . $item['approve_link']. '">' . __('Approve', 'approval-workflow'). '</a>' ;
        }
    	...
    	if(!empty($results)){
    		...
    		foreach($results as $post){
    			...
    			$this->items[] = array(
                        ...
                        'reviewer' => Approval_Workflow::render_reviewer_list( $reviewers ),
