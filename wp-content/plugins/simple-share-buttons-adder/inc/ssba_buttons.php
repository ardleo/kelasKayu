...
function updateSharedCount($socialMedia, $urlCurrentPage){
	// add to our db
	$sharedCount = getSharedCount($socialMedia, $urlCurrentPage);
	if ( $sharedCount > 0 ){
		if ( empty(get_post_meta(get_the_ID(), '_' . $socialMedia . 'SharedCount', true)) ){
			add_post_meta( get_the_ID(), '_' . $socialMedia . 'SharedCount',  $sharedCount );
		}else{
			update_post_meta( get_the_ID(), '_' . $socialMedia . 'SharedCount',  $sharedCount );
		}
	}
}

function getSharedCount($socialMedia, $urlCurrentPage){
	switch($socialMedia){
		case 'facebook':
			return getFacebookShareCount($urlCurrentPage);
		break;
		case 'twitter':
			return getTwitterShareCount($urlCurrentPage);
		break;
		case 'G+':
			return getGoogleShareCount($urlCurrentPage);
		break;
		case 'linkedin':
			return getLinkedinShareCount($urlCurrentPage);
		case 'pinterest':
			return getPinterestShareCount($urlCurrentPage);
		break;
		case 'reddit':
			return getRedditShareCount($urlCurrentPage);
		break;
		case 'tumblr':
			return getTumblrShareCount($urlCurrentPage);
		break;
		case 'yummly':
			return getYummlyShareCount($urlCurrentPage);
		break;
		case 'stumbleupon':
			return getStumbleUponShareCount($urlCurrentPage);
		break;
		default:
			return 0;
	}
...
function ssba_reddit($arrSettings, $urlCurrentPage, $strPageTitle, $booShowShareCount) {
    ...
    if ($arrSettings['ssba_show_share_count'] == 'Y' && $booShowShareCount == true) {
		...
		// add to our db
		updateSharedCount('reddit', $urlCurrentPage);
...
function ssba_tumblr($arrSettings, $urlCurrentPage, $strPageTitle, $booShowShareCount) {
    ...
    if ($arrSettings['ssba_show_share_count'] == 'Y' && $booShowShareCount == true) {
		...
		// add to our db
		updateSharedCount('tumblr', $urlCurrentPage);
...
function ssba_yummly($arrSettings, $urlCurrentPage, $strPageTitle, $booShowShareCount) {
    ...
    if ($arrSettings['ssba_show_share_count'] == 'Y' && $booShowShareCount == true) {
		...
		// add to our db
		updateSharedCount('yummly', $urlCurrentPage);
